...
- **The manuscript lives at `data/overleaf/template/_extracted/templateArxiv.tex`** (~1790 lines after revision 2) — that is the live copy. Always edit `_extracted/`, never the zips: the zips are numbered snapshots built *from* `_extracted/` for Overleaf upload. `_v2.zip` is the as-submitted version the reviewers saw, worth keeping as the historical record; `_v6.zip` is the revision-1 snapshot. **As of revision 2 no zip matches the working tree** — `_extracted/` is ahead of `_v6.zip`, and `_v7.zip` has not been built yet. Build the next numbered zip from `_extracted/` before any Overleaf upload; a stale zip uploaded there silently reverts the manuscript.
