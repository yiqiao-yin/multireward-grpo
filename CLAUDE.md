...
- **The manuscript lives at `data/overleaf/template/_extracted/templateArxiv.tex`** (~1660 lines after revision 1) — that is the live copy. Always edit `_extracted/`, never the zips: the zips are numbered snapshots built *from* `_extracted/` for Overleaf upload, and only the highest-numbered one matches the working tree (`_v6.zip` as of revision 1; `_v2.zip` is the as-submitted version the reviewers saw, worth keeping as the historical record). After changing the `.tex`, build a new numbered zip — a stale zip uploaded to Overleaf silently reverts the manuscript.
- **Submission documents live in `data/overleaf/`**: `reviewer_feedback.docx` (point-by-point response to the editor and both reviewers) with its derived `.pdf`, `Cover_Letter_Scientific_Reports.docx`/`.pdf`, and `Author_Contributions_Statement.txt`. The `.docx` files are generated by scripts in `data/tools/` — edit the script and regenerate, do not hand-edit the `.docx`, or the next regeneration silently discards the edit. It states the same claims as the tracked `.md` prose files, so **a numerical result that changes must be updated in both** — the `.tex` is in the private repo and the `.md` in the public one, so nothing will flag the drift for you. Commit `data/` changes from inside `data/` (its own repo); commit code changes from the root.
